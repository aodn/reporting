...
FROM d_1 GROUP BY channel_id)
  SELECT DISTINCT m.platform_code AS site_name, 
	...
	round((date_part('days', (m.time_coverage_end - m.time_coverage_start)) + date_part('hours', (m.time_coverage_end - m.time_coverage_start))/24)::numeric/365.25, 1) AS coverage_duration, 
