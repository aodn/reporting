...
CREATE OR REPLACE VIEW aatams_acoustic_species_all_deployments_view AS SELECT * FROM animal_tracking_acoustic_reporting.animal_tracking_acoustic_species_all_deployments_view;
...
no_transmitters, no_releases, no_releases_with_location, no_embargo, no_protected, latest_embargo_date, total_no_detections, no_detections_public, earliest_detection, latest_detection, no_data_days
 FROM animal_tracking_acoustic_reporting.animal_tracking_acoustic_species_data_summary_view
 ORDER BY registered, common_name;
CREATE OR REPLACE VIEW aatams_acoustic_embargo_totals_view AS SELECT * FROM animal_tracking_acoustic_reporting.animal_tracking_acoustic_embargo_totals_view;
CREATE OR REPLACE VIEW aatams_acoustic_registered_totals_view AS SELECT * FROM animal_tracking_acoustic_reporting.animal_tracking_acoustic_registered_totals_view;
CREATE OR REPLACE VIEW aatams_acoustic_stats_view AS SELECT * FROM animal_tracking_acoustic_reporting.animal_tracking_acoustic_stats_view;
CREATE OR REPLACE VIEW aatams_acoustic_project_all_deployments_view AS SELECT * FROM animal_tracking_acoustic_reporting.animal_tracking_acoustic_project_all_deployments_view ORDER BY project_name, installation_name, station_name;
CREATE OR REPLACE VIEW aatams_acoustic_project_data_summary_view AS SELECT * FROM animal_tracking_acoustic_reporting.animal_tracking_acoustic_project_data_summary_view;
CREATE OR REPLACE VIEW aatams_acoustic_project_totals_view AS SELECT * FROM animal_tracking_acoustic_reporting.animal_tracking_acoustic_project_totals_view;
